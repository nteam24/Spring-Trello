...
[블로그 정리](https://minjooig.tistory.com/148)
...
- 젠킨스를 통해 CICD 과정을 진행할 인스턴스 1개, 배포할 서비스가 동작할 인스턴스 1개를 준비해야한다.
- 서비스할 애플리케이션은 로컬 -> 깃허브 -> 젠킨스 EC2 -> 서비서 EC2 로 움직인다.
   - 만약 개인 프로젝트라면 젠킨스를 로컬에서 구현해도 가능할것이라 생각한다.
- 개발자가 CI, CD로 구현할 동작을 파이프라인을 통해 순서대로 작성하면, 젠킨스를 파이프라인의 순서를 준수하여 명령들을 실행한다.
